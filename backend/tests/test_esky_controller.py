...
from tests.fixtures.esky import *
...
def test_format_booking(formatted_booking):
    workbook = pd.ExcelFile("tests/test_data/esky/test_report.xlsx")
...
def test_get_bookings_for_stats_admin(order_numbers, report_file):
    response = client.post("/api/esky/", data=order_numbers, files=report_file)
    assert response.status_code == 200
    with open("tests/test_data/esky/test_csv_file.csv") as example_file:
